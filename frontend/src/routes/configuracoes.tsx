...
import type { CardPref } from "@/lib/relmeg/store";
...
function Configuracoes() {
  ...
  async function salvarSenha(e: React.FormEvent) {
    ...
    if (!(await alterarSenha((sessao as any)?.login, senhaAtual, novaSenha))) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="panel panel-hover rounded-xl p-6">
        ...
        <p className="text-sm text-muted-foreground">
          Altere a senha do usuário <strong className="text-foreground">{(sessao as any)?.login ?? "—"}</strong>.
        </p>
