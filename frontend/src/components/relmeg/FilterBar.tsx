...
/** Função utilitária de filtros que faltava */
export function aplicarFiltros(data: Parlamentar[], f: Filters): Parlamentar[] {
  return data.filter((p) => {
    if (f.busca) {
      const q = f.busca.toLowerCase();
      const texto = `${p.nome} ${p.partido} ${p.uf} ${p.cargo} ${setoresDe(p).join(" ")} ${temasInteresseDe(p).join(" ")} ${temasContrariosDe(p).join(" ")}`.toLowerCase();
